...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, args: dict, split='train'):
        ...
        if args['name'] == 'YouTubeVOS':
            vid_lst_prefix = args['train_root']
    ...
    def load_item(self, index):
        ...
        # create sample index
        ref_index = self._sample_index(len(all_frames), self.num_local_frames, self.num_ref_frames)
        ...
        print()
        midas_frame_tensors = torch.from_numpy(np.stack([self.midas_transform()({"image": np.array(x).astype(np.float)/255.})["image"] for x in frames], axis=0))
        mask_tensors = self._to_tensors(masks)
        return frame_tensors, midas_frame_tensors, mask_tensors
    
    def midas_transform(self):
        net_w, net_h = 384, 384
        resize_mode = "minimal"
